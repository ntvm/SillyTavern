...
router.post('/get', jsonParser, (request, response) => {
    ...
    //repeat same
    const NvPresets = readAndParseFromDirectory(request.user.directories.NvSettings);
    const ProxyManager = readAndParseFromDirectory(request.user.directories.ProxyManager);
