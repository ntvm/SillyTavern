Мини инструкция. Lookaround работает из коробки.; AlwaysCharnames(На данный момент находится в самом начале public\scripts\openai.js)(Включает перманентное имя карточки  в промпте) включено; В корне Таверны, config.conf: HumAssistOff (Убирает'Human:' и 'Assistant из таверны') выключен.;  Systemful (Возвращает роль System для Claude) включено; В интерфейсе Summarize встроенная возможность переключть оригинальный Summarize и вариант Hints от XML-кунов
