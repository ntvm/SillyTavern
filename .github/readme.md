...
Поддержка проксей для OAI Dall-E. (после включения галочки "Use instead key", Нажать шестеренку, это запишит проксю и пароль для использования. После этого можно вернуть свой пресет для текствовых ответов. Для удаления, выкдючите "Use instead key" и нажмите шестерню.);
