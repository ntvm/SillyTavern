...
async function sendOpenAIRequest(type, messages, signal) {
    ...
    switch (extension_settings.ProxyManager.ProxyPrior) {
        ...
        case true:
            if ([chat_completion_sources.CLAUDE, chat_completion_sources.OPENAI, chat_completion_sources.MISTRALAI, chat_completion_sources.MAKERSUITE].includes(oai_settings.chat_completion_source)) {
    ...
    if (isClaude) {
        ...
        if (!oai_settings.exclude_assistant || !extension_settings.Nvkun.exclude_Prefill) {
    ...
    if (stream) {
        ...
        return async function* streamData() {
            ...
            if (isOAI && oai_settings.openai_model == 'o1'){
                const data = await response.json(); checkQuotaError(data); checkModerationError(data);
                if (data.error) {
